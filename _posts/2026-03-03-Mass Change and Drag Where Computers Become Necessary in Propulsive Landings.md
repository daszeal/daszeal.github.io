...
I attributed this error to the second limitation discussed in my [first model](link): atmospheric drag and mass change. This led me to find ways to further improve [Name] by including mass changes and drag, outlined below.
...
- $$C_d$$ is just hard to work with. Most rocket-shaped objects with their engines pointing down have a Cd of 0.8-1.2[site smth], but that spikes when the vehicle goes from supersonic to transonic. This is very bad since most Falcon 9 landings happen around that band. And you can’t “calculate” it either: you can guess or you can use a wind tunnel/computational fluid dynamics.
...
where $$h$$ is the current altitude above sea level, the input, and $$H$$ is the *scale height*, or a height where the entire equation works out when put to the power of $$e$$. This value is around 8.5 km for Earth and [5km for Kerbin]().
...
The warning system also addresses the first limitation outlined in [my first post]() on this project, creating a way for real liquid-fueled engines to use this.
